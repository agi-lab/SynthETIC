...
##
## Regenerates both the standard-scale snapshot (~4k claims) and the stress
## snapshot (~40k claims, 10x larger) from the current SynthETIC source.
##
...
##
## Commit both `tests/testthat/cross_platform_snapshot.rds` and
## `tests/testthat/cross_platform_stress_snapshot.rds`.
##
...
## Stress snapshot (~40,000 claims, 10x larger) -- covers boundary-sensitive
## samplers at higher N where a 1-ULP findInterval flip is more likely to
## surface if any residual OS-dependent path remains.
stress <- gen_snapshot(seed = 20260712L, n_periods = 40L, lambda = 1000)
out_stress <- file.path("tests", "testthat", "cross_platform_stress_snapshot.rds")
saveRDS(stress, out_stress)
cat("Stress snapshot:  ", sum(stress$freq), "claims,",
    sum(unlist(stress$no_payments)), "payments ->", out_stress, "\n")